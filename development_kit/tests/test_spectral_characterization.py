...
def _bundle(values: list[float], wavelengths: list[float] | None = None):
    ...
    return build_spectral_point_bundle(
        ...
        rows=[
            _row(index, wavelength, value)
            for index, (wavelength, value) in enumerate(zip(wavelengths, values))
        ],
    )
...
def test_public_policy_normalizers_are_canonical_and_fail_closed():
    ...
    with pytest.raises(ValueError, match="fields"):
        normalize_spectral_measurement_configuration({**measurement, "automatic_acceptance": True})
    ...
    [
        (
            lambda rows: rows.__setitem__(
                1, {**rows[1], "requested_wavelength_m": rows[0]["requested_wavelength_m"]}
            ),
            "sorted and unique",
        ),
        (lambda rows: rows.__setitem__(2, {**rows[2], "A": float("nan")}), "finite"),
        (
            lambda rows: rows.__setitem__(2, {**rows[2], "configuration_sha256": "c" * 64}),
            "does not match",
        ),
        (
            lambda rows: rows.__setitem__(
                2, {**rows[2], "raw_row_sha256": rows[1]["raw_row_sha256"]}
            ),
            "raw row hashes",
        ),
    ],
...
def test_irregular_grid_is_valid_and_policy_tolerances_change_only_the_decision():
    ...
    flat = build_spectral_analysis_decision(bundle, _policy(flat_response_abs_tolerance=1.0))
...
def _characterize(values, *, measurement=None, wavelengths=None):
    ...
    result = build_spectral_characterization(bundle, decision, measurement or _measurement())
...
def test_measured_grid_peak_half_prominence_and_quality_factor_are_hash_bound():
    ...
    bundle, decision, result = _characterize([0.1, 0.5, 0.9, 0.5, 0.1], wavelengths=wavelengths)
    ...
    assert result["evidence_binding"]["rows"] == [
        {"row_id": row["row_id"], "raw_row_sha256": row["raw_row_sha256"]} for row in bundle["rows"]
    ]
...
def test_missing_half_prominence_crossing_is_explicit_and_quality_factor_is_absent():
    _bundle_value, _decision, result = _characterize(
        [0.7, 0.8, 0.9, 0.8, 0.7],
        measurement=_measurement(baseline_rule="declared_response", baseline_response_value=0.0),
    )
...
def test_fit_support_sensitivity_preserves_each_measurement_and_reports_spans():
    ...
    wavelengths = [4.8e-6 + index * 0.01e-6 for index in range(45)]
    values = [
        0.1 + 0.8 / (1.0 + ((wavelength - center) / 0.045e-6) ** 2) for wavelength in wavelengths
    ]
    _bundle_value, _decision, result = _characterize(
        values,
        wavelengths=wavelengths,
...
def test_measurement_configuration_rejects_unhashable_types_as_validation_errors(changes):
    configuration = _fit_measurement("local_polynomial_fit", 7, [])
    configuration.update(changes)

    with pytest.raises(ValueError):
        normalize_spectral_measurement_configuration(configuration)


def test_nonpositive_peak_prominence_is_not_reported_as_measured():
    _bundle_value, _decision, result = _characterize(
        [0.1, 0.5, 0.9, 0.5, 0.1],
        measurement=_measurement(
            baseline_rule="declared_response",
            baseline_response_value=1.0,
        ),
    )

    assert result["measurement_state"] == "not_measured"
    assert result["reason_code"] == "nonpositive_peak_prominence"


def test_fit_quality_policy_rejects_a_fit_with_nonzero_relative_residual():
    wavelengths = [4.8e-6 + index * 0.01e-6 for index in range(45)]
    values = [
        0.1
        + 0.8 / (1.0 + ((wavelength - 5.01e-6) / 0.04e-6) ** 2)
        + (1.0e-5 if index % 2 else -1.0e-5)
        for index, wavelength in enumerate(wavelengths)
    ]
    measurement = _fit_measurement("lorentzian_fit", 31, [])
    measurement["fit_quality_policy"]["maximum_relative_rms_residual"] = 0.0
    ...
    assert result["measurement_state"] == "not_measured"
    assert result["reason_code"] == "peak_fit_failed"
    assert "quality policy" in result["candidate"]["failure_reason"]


def test_characterization_binds_its_numerical_backend_identity():
    bundle, decision, result = _characterize([0.1, 0.5, 0.9, 0.5, 0.1])
    backend = result["algorithm"]["numerical_backend"]

    assert backend["numpy_version"]
    assert backend["scipy_version"]
    assert set(backend) == {
        "numpy_version",
        "scipy_version",
        "blas_name",
    ...
    tampered = deepcopy(result)
    tampered["algorithm"]["numerical_backend"]["numpy_version"] = "different"
    with pytest.raises(ValueError, match="noncanonical"):
        validate_spectral_characterization(tampered, bundle=bundle, decision=decision)


def test_derived_spectral_arithmetic_rejects_finite_input_overflow():
    bundle = _bundle([0.1, 0.2, 0.8, 0.2, 0.1])
    rows = deepcopy(bundle["rows"])
    rows[0].update({"R": -1.0e308, "T": -1.0e308, "A": -1.0e308})
    extreme = build_spectral_point_bundle(
        bundle_id=bundle["bundle_id"],
        source_model=bundle["source_model"],
        configuration_sha256=bundle["configuration_sha256"],
        parameter_state=bundle["parameter_state"],
    ...
    with pytest.raises(ValueError, match="arithmetic exceeds finite bounds"):
        build_spectral_analysis_decision(extreme, _policy())
...
def test_public_tool_returns_three_separate_hash_bound_artifacts():
    ...
    assert (
        result["candidate_measurements"]["decision_sha256"]
        == result["analysis_decision"]["decision_sha256"]
    )
