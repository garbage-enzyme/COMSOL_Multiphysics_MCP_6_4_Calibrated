...
def _spec(tmp_path, *, early: bool, cap: bool = False, tolerance: float = 10e-9):
    ...
    for level in raw["levels"]:
        level["spectral_job"]["measurement_configuration"]["peak_method"] = (
...
def test_fault_after_level_row_resumes_without_duplicate_spectrum(tmp_path):
    ...
    with pytest.raises(RuntimeError, match="injected"):
        run_convergence_campaign(spec, root, attempt=1, level_executor=execute, fault_hook=fault)
    assert calls == ["mesh-0"]

    result = run_convergence_campaign(spec, root, attempt=2, level_executor=execute)
    assert result["completed"] is True
...
def test_resume_replays_durable_level_notification_before_new_execution(tmp_path):
    spec = _spec(tmp_path, early=False, tolerance=20e-9)
    root = tmp_path / "campaign-callback-gap"
    calls = []
    notifications = []
    ...
    def fail_notification(row):
        raise RuntimeError(f"lost notification for {row['level_id']}")

    with pytest.raises(RuntimeError, match="lost notification"):
        run_convergence_campaign(
            spec,
            root,
            attempt=1,
            level_executor=execute,
            on_durable_level=fail_notification,
        )
    ...
    result = run_convergence_campaign(
        ...
        attempt=2,
        level_executor=execute,
        on_durable_level=lambda row: notifications.append(row["level_id"]),
    )
