...
@pytest.mark.parametrize(
    ...
    [
        ...
        (
            lambda value: value["levels"][2].__setitem__("declared_predecessor_level_id", "mesh-0"),
            "adjacency",
        ),
        (
            lambda value: value["levels"][1].__setitem__("material_identity_sha256", "f" * 64),
            "material identity",
        ),
        (
            lambda value: value["levels"][1].__setitem__("incidence_identity_sha256", "f" * 64),
            "incidence identity",
        ),
        (
        ...
        (
            lambda value: value["convergence_policy"]["metrics"][0].__setitem__("unit", "nm"),
            "unit",
        ),
        (
        ...
        (
            lambda value: value["stop_policy"].__setitem__("minimum_completed_levels", 2),
            "cannot precede",
        ),
...
def test_duplicate_exact_model_bytes_and_configuration_identities_fail_closed(tmp_path):
    ...
    second_source = Path(raw["levels"][1]["spectral_job"]["source_model_path"])
    ...
    raw = _raw_campaign(tmp_path)
    raw["levels"][1]["spectral_job"]["configuration_sha256"] = raw["levels"][0]["spectral_job"][
        "configuration_sha256"
    ]
