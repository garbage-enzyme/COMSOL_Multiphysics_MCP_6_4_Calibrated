"""Licensed probe: resolve how a trained DNN prediction can be obtained.

The S8 campaign gate must screen candidates with the *real* trained model.  No
documented reference states which call evaluates a trained COMSOL DNN function,
so this probe trains a small network and then attempts every plausible
evaluation path, recording the exact outcome of each.
"""
...
SCHEMA_NAME = "comsol_mcp.surrogate_prediction_eval_probe"
...
def _run_worker(output: Path, cores: int, workspace: Path) -> int:
    ...
    try:
        ...
        dataset_path = workspace / "prediction_eval_probe.csv"
        _write_dataset(dataset_path)

        model = client.create("PredictionEvalProbe")
        jm = model.java
        jm.param().set("a1", "1.0")
        jm.param().set("a2", "2.0")
        ...
        # ---- Attempt 1: evaluate the DNN function name as a COMSOL function.
        attempts: list[dict[str, Any]] = []

        def attempt(name: str, call, *, needs_solve: bool = False) -> None:
            entry: dict[str, Any] = {"path": name, "needs_solve": needs_solve}
            try:
                value = call()
                entry["ok"] = True
                entry["value"] = None if value is None else str(value)[:200]
            except Exception as exc:
                entry["ok"] = False
                entry["error"] = f"{type(exc).__name__}: {exc}"[:400]
            attempts.append(entry)

        # The DNN function registers COMSOL function names; evaluating one of
        # those names at the parameter values is the natural prediction path.
        attempt("func.functionNames()", lambda: [str(x) for x in list(dnn.functionNames())])

        jm.param().set("a1", "1.0")
        jm.param().set("a2", "2.0")
        for candidate_name in ("dnn1", "dnn1_qoi", "qoi"):
            attempt(
                f"jm.evaluate('{candidate_name}')",
                lambda n=candidate_name: jm.evaluate(n),
            )
            attempt(
                f"model.evaluate('{candidate_name}')",
                lambda n=candidate_name: model.evaluate(n),
            )
            attempt(
                f"jm.numerical().evaluate('{candidate_name}')",
                lambda n=candidate_name: jm.numerical().evaluate(n),
            )

        # The exported ONNX names a COMSOL function `dnn1`; check the registered
        # function list for the real names before concluding.
        attempt("dnn.getType()", lambda: dnn.getType())
        attempt("dnn.getString('funcs')", lambda: dnn.getString("funcs"))
        attempt("dnn.getString('args')", lambda: dnn.getString("args"))
        attempt("dnn.getString('information')", lambda: dnn.getString("information"))
...
def _run_parent(args) -> int:
    ...
    owner = SolverOwnership(args.runtime_root, owner="surrogate-prediction-eval-probe")
    ...
    try:
        ...
        claim = owner.acquire(mode="surrogate-prediction-eval-probe")
