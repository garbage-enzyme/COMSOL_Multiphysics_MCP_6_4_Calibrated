...
ZH_CN = {
    ...
    "Isolated Electrochemistry Module profile. Adds electrochemistry discovery, interface creation, electrode reaction, and electrolyte tools to Core. These tools are not available in the default or Full profiles.": "隔离的电化学模块 profile。在 Core 基础上增加电化学发现、接口创建、电极反应和电解质工具。这些工具在默认或 Full profile 中不可用。",
    ...
    "Offline model inspection for computers without COMSOL. Only solver-free read-only inspection tools are available; nothing here can start COMSOL or Java.": "面向未安装 COMSOL 的计算机的离线模型检查。仅提供求解器无关的只读检查工具；此处任何功能都不会启动 COMSOL 或 Java。",
    ...
    "Manual sources and lexical search": "手册来源与词法搜索",
    "Optional semantic search": "可选语义搜索",
    "Enable local keyword search for installed or copied PDF manuals. This is off by default because COMSOL manuals may not be installed.": "启用已安装或复制到本机的 PDF 手册关键词搜索。由于安装 COMSOL 时可能未安装手册，此功能默认关闭。",
    "Folder scanned recursively when generating the manual index. Choose the folder containing the PDF manuals.": "生成手册索引时递归扫描的文件夹。请选择包含 PDF 手册的文件夹。",
    "SQLite FTS5/BM25 index used by manual_search. Enter an ASCII-only SQLite file or destination folder. A folder uses lexical_manuals.sqlite3.": "manual_search 使用的 SQLite FTS5/BM25 索引。请输入仅含 ASCII 字符的 SQLite 文件或目标文件夹；文件夹会使用 lexical_manuals.sqlite3。",
    ...
    "Optional folder containing the prepared semantic vector index. It is independent from the SQLite lexical index.": "存放已准备语义向量索引的可选文件夹。它与 SQLite 词法索引相互独立。",
    ...
    "Generate Index": "生成索引",
    "Generate manual index": "生成手册索引",
    "Starting index worker...": "正在启动索引工作进程……",
    "Cancelling safely...": "正在安全取消……",
    "{percent}% — {files}/{total_files} PDF(s), {pages}/{total_pages} page(s){source}": "{percent}% — {files}/{total_files} 个 PDF，{pages}/{total_pages} 页{source}",
    "Replace existing index?": "要替换现有索引吗？",
    "A validated new index will atomically replace the existing file. Continue?": "验证通过的新索引将原子替换现有文件。是否继续？",
    "Choose both the PDF folder and SQLite index file first.": "请先同时选择 PDF 文件夹和 SQLite 索引文件。",
    "Index generation could not start. Check the selected paths and permissions.": "无法启动索引生成。请检查所选路径和权限。",
    "Manual index ready": "手册索引已就绪",
    "Indexed {pdfs} PDF(s) and {pages} page(s). Corpus fingerprint: {fingerprint}": "已索引 {pdfs} 个 PDF、{pages} 页。语料指纹：{fingerprint}",
    "Index generation failed": "索引生成失败",
    "The index worker stopped without a completion receipt.": "索引工作进程已停止，但未返回完成凭据。",
    "Index generation cancelled": "已取消索引生成",
    "Temporary files were removed and the previous index was preserved.": "临时文件已清理，原有索引保持不变。",
    "Manual search is not ready": "手册搜索尚未就绪",
    "Choose or generate a valid SQLite manual index before enabling search.": "请先选择或生成有效的 SQLite 手册索引，再启用搜索。",
    "Semantic search is not ready": "语义搜索尚未就绪",
    "Enable manual search and choose valid lexical, semantic-index, and model assets first.": "请先启用手册搜索，并选择有效的词法索引、语义索引和模型资源。",
}
...
ZH_TW = {
    ...
    "Repositories and acknowledgements": "儲存庫與致謝",
    "This repository": "本儲存庫",
    ...
    "Isolated Electrochemistry Module profile. Adds electrochemistry discovery, interface creation, electrode reaction, and electrolyte tools to Core. These tools are not available in the default or Full profiles.": "隔離的電化學模組 profile。在 Core 基礎上增加電化學發現、介面建立、電極反應和電解質工具。這些工具在預設或 Full profile 中不可用。",
    ...
    "Offline model inspection for computers without COMSOL. Only solver-free read-only inspection tools are available; nothing here can start COMSOL or Java.": "為未安裝 COMSOL 的電腦提供的離線模型檢查。僅提供求解器無關的唯讀檢查工具；此處任何功能都不會啟動 COMSOL 或 Java。",
    "Folder for working files and locks. Use an ASCII-only path. \nExample: %PROGRAMDATA%\\comsol_mcp\\runtime": "存放工作檔案和鎖定資料的資料夾。路徑只能含 ASCII 字元。\n範例：%PROGRAMDATA%\\comsol_mcp\\runtime",
    ...
    "Manual sources and lexical search": "手冊來源與詞彙搜尋",
    "Optional semantic search": "可選語意搜尋",
    "Enable local keyword search for installed or copied PDF manuals. This is off by default because COMSOL manuals may not be installed.": "啟用已安裝或複製到本機的 PDF 手冊關鍵字搜尋。由於安裝 COMSOL 時可能未安裝手冊，此功能預設關閉。",
    "Folder scanned recursively when generating the manual index. Choose the folder containing the PDF manuals.": "產生手冊索引時遞迴掃描的資料夾。請選擇包含 PDF 手冊的資料夾。",
    "SQLite FTS5/BM25 index used by manual_search. Enter an ASCII-only SQLite file or destination folder. A folder uses lexical_manuals.sqlite3.": "manual_search 使用的 SQLite FTS5/BM25 索引。請輸入僅含 ASCII 字元的 SQLite 檔案或目標資料夾；資料夾會使用 lexical_manuals.sqlite3。",
    ...
    "Optional folder containing the prepared semantic vector index. It is independent from the SQLite lexical index.": "存放已準備語意向量索引的可選資料夾。它與 SQLite 詞彙索引相互獨立。",
    ...
    "Generate Index": "產生索引",
    "Generate manual index": "產生手冊索引",
    "Starting index worker...": "正在啟動索引工作程序……",
    "Cancelling safely...": "正在安全取消……",
    "{percent}% — {files}/{total_files} PDF(s), {pages}/{total_pages} page(s){source}": "{percent}% — {files}/{total_files} 個 PDF，{pages}/{total_pages} 頁{source}",
    "Replace existing index?": "要取代現有索引嗎？",
    "A validated new index will atomically replace the existing file. Continue?": "驗證通過的新索引將原子取代現有檔案。是否繼續？",
    "Choose both the PDF folder and SQLite index file first.": "請先同時選擇 PDF 資料夾和 SQLite 索引檔案。",
    "Index generation could not start. Check the selected paths and permissions.": "無法啟動索引產生。請檢查所選路徑和權限。",
    "Manual index ready": "手冊索引已就緒",
    "Indexed {pdfs} PDF(s) and {pages} page(s). Corpus fingerprint: {fingerprint}": "已索引 {pdfs} 個 PDF、{pages} 頁。語料指紋：{fingerprint}",
    "Index generation failed": "索引產生失敗",
    "The index worker stopped without a completion receipt.": "索引工作程序已停止，但未回傳完成憑據。",
    "Index generation cancelled": "已取消索引產生",
    "Temporary files were removed and the previous index was preserved.": "暫存檔案已清理，原有索引保持不變。",
    "Manual search is not ready": "手冊搜尋尚未就緒",
    "Choose or generate a valid SQLite manual index before enabling search.": "請先選擇或產生有效的 SQLite 手冊索引，再啟用搜尋。",
