...
_EXPRESSION_NAME = re.compile(r"^[A-Za-z][A-Za-z0-9_]*(?:\.[A-Za-z][A-Za-z0-9_]*)*$")
_C = 299_792_458.0
...
def _angular_expression(value: float, unit: str | None, name: str | None) -> str:
    ...
    if normalized in _FREQUENCY_FACTORS:
        if name is not None:
            return f"(2*pi*{base})"
        return f"(2*pi*{base}[{_FREQUENCY_SYMBOLS[normalized]}])"
    if name is not None:
        return base
    return f"({base}[1/s])"
...
def preview_material_expression(
    *,
    ...
    if harmonic_convention not in {"exp(+i*omega*t)", "exp(-i*omega*t)"}:
        raise ValueError(
            "harmonic_convention must explicitly be exp(+i*omega*t) or exp(-i*omega*t)"
        )
    ...
    if model_kind == "n_k":
        values["refractive_index"] = _number(
            parameters["refractive_index"], "parameters.refractive_index", nonnegative=True
        )
    if model_kind == "drude":
        values["plasma_angular_frequency"] = _number(
            parameters["plasma_angular_frequency"],
            "parameters.plasma_angular_frequency",
            positive=True,
        )
        values["damping_angular_frequency"] = _number(
            parameters["damping_angular_frequency"],
            "parameters.damping_angular_frequency",
            nonnegative=True,
        )
    if model_kind == "lorentz":
        values["resonance_angular_frequency"] = _number(
            parameters["resonance_angular_frequency"],
            "parameters.resonance_angular_frequency",
            positive=True,
        )
        values["damping_angular_frequency"] = _number(
    ...
    for key in _ANGULAR_PARAMETERS & set(required):
        if key not in units:
            warnings.append(
                {"code": "missing_parameter_unit", "parameter": key, "assumed_unit": "1/s"}
            )
        converted_values[key] = _angular_value(
            values[key], units.get(key), f"parameter_units.{key}"
        )
    ...
    for key in set(units) - _ANGULAR_PARAMETERS:
        if units[key].strip().lower() not in {"1", "dimensionless"}:
            warnings.append(
                {
                    "code": "dimensionless_parameter_unit_ignored",
                    "parameter": key,
                    "unit": units[key],
                }
            )

    normalized_wavelength_unit = (
        wavelength_unit.strip().lower() if isinstance(wavelength_unit, str) else ""
    )
    if normalized_wavelength_unit not in _WAVELENGTH_FACTORS:
        raise ValueError("wavelength_unit must be m, um, µm, or nm")
    if (
        not isinstance(test_wavelengths, list)
        or not test_wavelengths
        or len(test_wavelengths) > MAX_PREVIEW_WAVELENGTHS
    ):
    ...
    for index, value in enumerate(test_wavelengths):
        wavelength_m = (
            _number(value, f"test_wavelengths[{index}]", positive=True)
    ...
    if frequency_source == "fixed_angular_frequency":
        if fixed_angular_frequency is None:
            raise ValueError(
                "fixed_angular_frequency is required for fixed_angular_frequency source"
            )
        ...
        omega_expression = _angular_expression(fixed_value, fixed_angular_frequency_unit, None)
        warnings.append(
            {
                "code": "frozen_frequency",
        ...
        warnings.append(
            {
                ...
                "message": (
                    "Preview assumes the physics frequency is synchronized to each test wavelength."
                ),
            }
        )
    else:
    ...
    for key in required:
        if key in _ANGULAR_PARAMETERS:
            parameter_expressions[key] = _angular_expression(
                values[key], units.get(key), names.get(key)
            )
        else:
    ...
    if model_kind == "constant":
        ...
        signed_index = _signed_imaginary(
            parameter_expressions["refractive_index"],
            parameter_expressions["extinction_coefficient"],
            imaginary_sign,
        )
        expression = f"{signed_index}^2"
    ...
    zero_loss = (
        ...
        or (
            model_kind in {"drude", "lorentz"}
            and converted_values["damping_angular_frequency"] == 0
        )
    )
    if zero_loss:
        warnings.append(
            {
                "code": "zero_loss_parameter",
                "message": (
                    "The sign diagnostic may be indeterminate because the declared "
                    "loss parameter is zero."
                ),
            }
        )
    warnings.append(
        {
            ...
            "message": (
                "The declared complex sign is not proof of physical passivity; use "
                "solved Qh and power closure."
            ),
        }
    )
    ...
    for requested, wavelength_m in zip(test_wavelengths, wavelengths_m):
        ...
        diagnostic = (
            ...
            if epsilon.imag == 0
                ...
                "matches_declared_sign"
                if math.copysign(1.0, epsilon.imag) == expected_direction
                else "opposes_declared_sign"
            )
        )
        preview.append(
            {
    ...
    ledger = {
        ...
        "parameter_units": {
            key: units.get(key, "1/s" if key in _ANGULAR_PARAMETERS else "1") for key in required
        },
        "named_parameter_unit_contract": "declared_units_already_attached",
    ...
    return {
        ...
        **configuration,
        ...
        "assessment": {
            ...
            "requires_solved_evidence": [
                "per-domain Qh",
                "passive R/T/A",
                "declared physical power closure",
            ],
        },
    }
