...
def execute_loaded_spectral_level(
    *,
    ...
    mesh_telemetry_diagnostics: list[dict[str, str]] = []
    ...
    def with_telemetry_diagnostics(decision: dict[str, Any]) -> dict[str, Any]:
        if mesh_telemetry_diagnostics:
            ...
            decision["telemetry_diagnostics"] = list(mesh_telemetry_diagnostics)
        return decision
    ...
    def resource_control(context: Mapping[str, Any]) -> dict[str, Any]:
        ...
        decision = with_telemetry_diagnostics(
    ...
    def resource_after(row: Mapping[str, Any]) -> dict[str, Any]:
        nonlocal latest_resource_decision
        decision = with_telemetry_diagnostics(
            resource.evaluate(stage="post_solve", point_id=str(row["point_id"]))
        )
        latest_resource_decision = decision
        return {
            "action": "continue"
    ...
    return {
        "result": result,
        "latest_resource_decision": latest_resource_decision,
        "telemetry_diagnostics": list(mesh_telemetry_diagnostics),
    }
