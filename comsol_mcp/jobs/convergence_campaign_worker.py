"""Detached worker for one durable exact-model convergence campaign."""
...
def _run(
    ...
    *,
    ...
    """Run one campaign attempt while preserving exact completed child levels."""
    ...
    try:
        ...
        preflight = ownership.preflight(
            ...
            output_path=str(directory / "convergence_levels.jsonl"),
        ...
        def point_persisted(level: Mapping[str, Any], row: Mapping[str, Any]) -> None:
            ...
            if current == "smoke_running":
                ...
                store.update_state(job_id, "running", event="convergence_campaign_started")
            store.update_state(
                job_id,
                patch={
                ...
                event_data={"level_id": level["level_id"], "point_id": row["point_id"]},
            )

        def execute_level(level: Mapping[str, Any], level_dir: Path) -> Mapping[str, Any]:
            ...
            if _sha256_file(source) != child["source_model_sha256"]:
                raise RuntimeError("Immutable convergence level source hash changed")
            client.clear()
            ...
            execution = execute_loaded_spectral_level(
                ...
                on_durable_row=lambda row: point_persisted(level, row),
            ...
            if _sha256_file(source) != child["source_model_sha256"]:
                raise RuntimeError("Immutable convergence level source changed after spectrum")
            return execution["result"]

        def level_persisted(row: Mapping[str, Any]) -> None:
            store.update_state(
                job_id,
                patch={
                    "completed_levels": row["ordinal"] + 1,
                    "last_level": {
                        "level_id": row["level_id"],
                        "row_sha256": row["row_sha256"],
                    },
                },
                event="durable_convergence_level",
                event_data={"level_id": row["level_id"], "row_sha256": row["row_sha256"]},
            )
        ...
        if should_stop():
            ...
            return 0
        store.update_state(job_id, "smoke_running", event="convergence_campaign_worker_started")
        result = run_convergence_campaign(
            ...
            on_durable_level=level_persisted,
            fault_hook=fault_hook,
        )
        if should_stop() or result.get("stop_reason") in {
            ...
            pending_terminal = {
                ...
                "patch": {
                    ...
                    "completed_levels": result["summary"]["completed_level_count"],
