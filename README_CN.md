...
当前 dependency/process-only 门槛为 **956 passed, 13 deselected**。单元测试无副作用：测试收集不会启动 COMSOL；integration probe 仅在显式请求时运行，并在全新的串行子进程中对精确进程树进行清理。仓库专用测试、release fixture、gate 与 provenance 见 `development_kit/README.md`；普通 wheel/sdist 不包含该目录。
