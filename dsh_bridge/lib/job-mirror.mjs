	...
	function write(rows) {
		try {
			...
			try { renameSync(tmp, file); } catch (e) {
				// Windows sharing violation: bounded single retry, then keep last good file
				log("warn", `state rename failed (${e.message}); retrying once`);
				setTimeout(() => { try { renameSync(tmp, file); } catch {} }, 50);
			}
		} catch (e) {
